...
# * sqlite setup (replaces postgres setup)
conn = sqlite3.connect('clanka.db') 
cursor = conn.cursor()
...
def getDB():
    # * sqlite connection to local file
    conn = sqlite3.connect('clanka.db', check_same_thread=False)
...
def storeInDb(userMessage, botResponse):
    ...
    VALUES (?, ?)''', (userMessage, botResponse))
